...
(function (global, factory) {
  ...
  const byTemplate = function (doc) {
    ...
    for (let i = 0; i < templates.length; i++) {
      ...
      if (mapping$3.hasOwnProperty(title)) {
        found.push({ reason: mapping$3[title], name: title });
      } else {
        // try regex-list on it
        let type = byPattern(title, patterns$2);
        if (type) {
  ...
  const byCategory = function (doc) {
    ...
    for (let i = 0; i < cats.length; i++) {
      ...
      if (mapping$2.hasOwnProperty(cat)) {
        found.push({ reason: mapping$2[cat], cat: cat });
        continue
      }
      // loop through our patterns
      let match = byPattern(cat, patterns$1);
      if (match) {
  ...
  const byTitle = function (doc) {
    ...
    if (mapping$1.hasOwnProperty(inside)) {
      return [{ reason: mapping$1[inside], title: inside }]
    }
